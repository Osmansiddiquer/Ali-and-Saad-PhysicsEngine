using Raylib_cs;
using PhysicsEngine.src.components;
using System.Numerics;

namespace PhysicsEngine.src.physics._2D.body;
public class PlayerBody : RigidBox2D {

    Camera2D camera;
    public PlayerBody(Vector2 position, float rotation, Vector2 scale, float width, float height, List<Component> components, Camera2D camera) : 
        base(position, rotation, scale, 70, 0.985f, width * height, 0f, width, height, components)
    {
        this.camera = camera;
        base.components.Add(new PlayerInput());
    }

}

